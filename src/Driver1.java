...
public class Driver1 {
	...
	public static void findLeastAndMostExpensiveUAV(Object[] flyingObjectsArray) {
		...
		for(int l = 0; l < flyingObjectsArray.length; l++) {
			if(flyingObjectsArray[l].getClass() == UAV.class || flyingObjectsArray[l].getClass() == MAV.class || flyingObjectsArray[l].getClass() == AgriculturalDrone.class) {
				...
				//NEW
				tempoUAVArray[l] = flyingObjectsArray[l];
				if(uavNb == 2) {
					boolean x = (((((UAV)(tempoUAVArray[l])).getPrice()) / (((UAV)(tempoUAVArray[l-1]).getPrice()))) == 0);
					boolean y = 2.0 / 4.0 == 0;
				}
				
				UAV objectInArray = null;
				UAV object = null;
				for(int q = l+1; q < flyingObjectsArray.length; q++) {
					objectInArray = (UAV)flyingObjectsArray[l];
					object = (UAV)flyingObjectsArray[q];
					if(objectInArray.getPrice() % object.getPrice() == 0) {
						//if object1 equals all other objects that means that object2 and object3 must equal, and object3 = object4, and so on, so they are in fact all the same price
					}
				}
	...
	public static void main(String[] args) {
		...
		//MAV Class
		MAV mav1 = new MAV();
		MAV mav2 = new MAV(100, 100000, "DJI Mini", 20.0);
		//AgriculturalDrone Class
		AgriculturalDrone agriD1  = new AgriculturalDrone();
		AgriculturalDrone agriD2 = new AgriculturalDrone(89.0, 2000000000, "AgriDBrand", 30);
